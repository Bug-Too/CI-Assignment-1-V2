...
def print_error(error_list: list[list[float]]):
    ...
    for current_error in error_list:
        ...
        print('root mean square error: ', sse ** 0.5)
...
def train(layers: list[int], bias: float, learning_rate: float, momentum_rate: float, max_epoch: int, epsilon: float, training_data: list[list[list[float]]], validation_data: list[list[list[float]]]) -> list[float]:
    ...
    while sum_square_error_average > epsilon and epoch_count < max_epoch:
        ...
        # print('Epoch:', epoch_count, 'SSE:', sum_square_error_average)
        epoch_count += 1
...
def network_test_param():
    ...
    """
    print('--------------------------------------------')
    error = cross_validate([8, 4, 4, 1], 1, 0.1, 0.1, 4000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
    print('--------------------------------------------')
    error = cross_validate([8, 4, 1], 1, 0.1, 0.05, 2000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
    print('--------------------------------------------')
    error = cross_validate([8, 1, 2, 1], 1, 0.05, 0.01, 2000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
    print('--------------------------------------------')
    error = cross_validate([8, 4, 2, 1], 1, 0.1, 0.05, 2000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
    print('--------------------------------------------')
    error = cross_validate([8, 4, 1], 1, 0.2, 0.1, 2000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
    print('--------------------------------------------')
    error = cross_validate([8, 4, 1], 1, 0.5, 0.1, 2000, 0.001, './Flood/Flood_dataset.txt')
    print_error(error)
